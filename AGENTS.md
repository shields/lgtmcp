...
LGTMCP is a Model Context Protocol server that reviews code changes using Google Gemini 3.8 Flash and either commits them (if approved) or returns review comments. Setup, configuration, usage, and troubleshooting are covered in `README.md`; every configuration option is documented in `config.example.yaml`.
...
- **Review Engine** (`internal/review/`) - Gemini 3.8 Flash integration with file retrieval
...
- **No sampling parameters**: `temperature`, `top_p`, and `top_k` are deliberately not sent (a `temperature` option was removed in 2026). Gemini 3 models do not support them — Google's Gemini 3.8 Flash migration checklist says to strip them — so `thinking_level` is the review's only generation knob.
...
- **Gemini 3.8 Flash**: Fast, capable model for code understanding
